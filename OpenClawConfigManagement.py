...
45. Version management PS1 scripts generated by _write_version_manager_scripts() (2026-05-20)
    install-openclaw.ps1, update-openclaw.ps1, openclaw-version-manager.ps1
    written to ~/.openclaw/workspace/ on every Apply-fixes.
    Stable pin 2026.3.28 + GitHub .tgz fallback embedded as constants.
    fix_openclaw_installation() uses stable pin as first install source.
...
"    → User melden, NICHT still ausweichen.\n"
    ...
    # ── DECISION #45: Stable version pin + GitHub fallback ──────────────────
    # 2026.3.28 is the last version where all config fixes work without retired workarounds.
    # The GitHub .tgz provides a pre-packaged install that bypasses npm entirely.
    OPENCLAW_STABLE_VERSION = "2026.3.28"
    OPENCLAW_GITHUB_FALLBACK = (
        "https://github.com/isonwillis/OpenClawWinInstaller/releases/"
        "download/v1.0.1/openclaw-2026.3.28.tgz"
    )

    def _write_version_manager_scripts(self) -> bool:
        """
        Generates version management PS1 scripts into the OpenClaw workspace.

        DECISION #45 (v1.0.5): install-openclaw.ps1, update-openclaw.ps1, and
        openclaw-version-manager.ps1 are written dynamically on every Apply-fixes
        run — consistent with how all other PS1/JS scripts are generated in this
        installer (patch_gateway_cmd, _write_skill_file, research_query.ps1, etc.).

        This ensures the scripts are always in sync with the current stable pin
        (OPENCLAW_STABLE_VERSION) and GitHub fallback URL. If the stable pin changes
        in a future DECISION, the scripts are automatically regenerated on the next
        Apply-fixes — no manual file update needed.

        Output location: ~/.openclaw/workspace/
          - install-openclaw.ps1
          - update-openclaw.ps1
          - openclaw-version-manager.ps1

        Returns True on success, False if the workspace directory is not available.
        """
        cfg_dir       = self._find_openclaw_config_dir()
        workspace_dir = os.path.join(cfg_dir, "workspace")
        os.makedirs(workspace_dir, exist_ok=True)

        stable  = self.OPENCLAW_STABLE_VERSION
        tgz_url = self.OPENCLAW_GITHUB_FALLBACK
        sep     = "\\"   # PS1 path separator — single backslash in generated output

        # ── install-openclaw.ps1 ─────────────────────────────────────────────
        install_lines = [
            "# install-openclaw.ps1 - generated by OpenClawWinInstaller (DECISION #45)",
            "# DO NOT EDIT - regenerated on every Apply-fixes run.",
            "# Installs OpenClaw at the pinned stable version (" + stable + ").",
            "# Falls back to GitHub .tgz if npm is unreachable.",
            "param(",
            '    [string]$Version       = "' + stable + '",',
            "    [switch]$ForceNewVersion",
            ")",
            "$STABLE_VERSION  = \"" + stable + "\"",
            "$GITHUB_FALLBACK = \"" + tgz_url + "\"",
            "",
            "function Write-Ok   { param($m) Write-Host \"  OK  $m\" -ForegroundColor Green }",
            "function Write-Warn { param($m) Write-Host \"  W   $m\" -ForegroundColor Yellow }",
            "function Write-Fail { param($m) Write-Host \"  ERR $m\" -ForegroundColor Red }",
            "",
            "if ($Version -ne $STABLE_VERSION -and -not $ForceNewVersion) {",
            "    Write-Warn \"Version $Version is newer than stable pin ($STABLE_VERSION).\"",
            "    Write-Warn \"Known issues > " + stable + ": 2026.4.x hardcoded 60s timeout,\"",
            "    Write-Warn \"2026.5.7 config wipe (issue #80077), 2026.5.18 exec schema strict.\"",
            "    Write-Warn \"Run Apply-fixes in the installer GUI after installing a newer version.\"",
            "    $c = Read-Host \"  Install $Version anyway? (yes/no)\"",
            "    if ($c -ne \"yes\") { Write-Warn \"Cancelled.\"; exit 1 }",
            "}",
            "",
            "Write-Host \"Removing existing OpenClaw...\"",
            "npm uninstall -g openclaw 2>$null | Out-Null",
            "",
            "if ($Version -eq $STABLE_VERSION) {",
            "    $ok = $false",
            "    Write-Host \"Installing openclaw@$Version from npm...\"",
            "    npm install -g \"openclaw@$Version\" 2>&1",
            "    if ($LASTEXITCODE -eq 0) { $ok = $true; Write-Ok \"npm install succeeded\" }",
            "    if (-not $ok) {",
            "        Write-Warn \"npm failed -- trying GitHub fallback: $GITHUB_FALLBACK\"",
            "        npm install -g $GITHUB_FALLBACK 2>&1",
            "        if ($LASTEXITCODE -eq 0) { Write-Ok \"GitHub fallback succeeded\" }",
            "        else { Write-Fail \"Both sources failed.\"; exit 1 }",
            "    }",
            "} else {",
            "    npm install -g \"openclaw@$Version\" 2>&1",
            ...
            "}",
            "try { Write-Ok \"Ready: $(openclaw --version 2>&1)\" }",
            "catch { Write-Warn \"Verify manually: openclaw --version\" }",
        ]
        install_ps1 = "\r\n".join(install_lines) + "\r\n"

        # ── update-openclaw.ps1 ─────────────────────────────────────────────
        script_dir_install = '"$SCRIPT_DIR' + sep + 'install-openclaw.ps1"'
        update_lines = [
            "# update-openclaw.ps1 - generated by OpenClawWinInstaller (DECISION #45)",
            "# DO NOT EDIT - regenerated on every Apply-fixes run.",
            "# Updates or repairs OpenClaw. Delegates to install-openclaw.ps1.",
            "param([string]$TargetVersion = \"" + stable + "\", [switch]$Force)",
            "$STABLE_VERSION = \"" + stable + "\"",
            "$SCRIPT_DIR     = Split-Path -Parent $MyInvocation.MyCommand.Path",
            "",
            "function Parse-V { param($v)",
            "    if ($v -eq \"latest\") { return [int]::MaxValue }",
            "    $p = ($v -replace 'v','') -split '[.]'",
            "    if ($p.Count -lt 3) { return 0 }",
            "    try { return [int]$p[0]*10000+[int]$p[1]*100+[int]$p[2] } catch { return 0 }",
            "}",
            "",
            "$cur = $null",
            "try { $cur = (openclaw --version 2>&1).Trim() } catch {}",
            "if (-not $cur) {",
            "    Write-Host \"OpenClaw not found - installing $TargetVersion...\"",
            "    & " + script_dir_install + " -Version $TargetVersion; exit $LASTEXITCODE",
            "}",
            "if ($cur -eq $TargetVersion -and -not $Force) {",
            "    Write-Host \"  OK  Already on $TargetVersion\"; exit 0",
            "}",
            "Write-Host \"  Update: $cur -> $TargetVersion\"",
            "if ((Parse-V $TargetVersion) -gt (Parse-V $STABLE_VERSION) -and -not $Force) {",
            "    Write-Host \"  W   Target $TargetVersion > stable pin ($STABLE_VERSION).\" -ForegroundColor Yellow",
            "    Write-Host \"      Run Apply-fixes in installer GUI after update.\" -ForegroundColor Yellow",
            "    $c = Read-Host \"  Proceed? (yes/no)\"",
            "    if ($c -ne \"yes\") { exit 0 }",
            "}",
            "if ($TargetVersion -eq $STABLE_VERSION) {",
            "    & " + script_dir_install + " -Version $TargetVersion",
            "} else {",
            "    & " + script_dir_install + " -Version $TargetVersion -ForceNewVersion",
            "}",
            "exit $LASTEXITCODE",
        ]
        update_ps1 = "\r\n".join(update_lines) + "\r\n"
        ...
        # ── openclaw-version-manager.ps1 ───────────────────────────────────
        script_dir_install2 = '"$SCRIPT_DIR' + sep + 'install-openclaw.ps1"'
        script_dir_update   = '"$SCRIPT_DIR' + sep + 'update-openclaw.ps1"'
        manager_lines = [
            "# openclaw-version-manager.ps1 - generated by OpenClawWinInstaller (DECISION #45)",
            "# DO NOT EDIT - regenerated on every Apply-fixes run.",
            "# Unified version management CLI.",
            "param(",
            "    [Parameter(Position=0)]",
            "    [ValidateSet(\"install\",\"update\",\"switch\",\"status\",\"list\")]",
            ...
            "switch ($Command) {",
            "    \"status\" {",
            "        Write-Host \"OpenClaw Version Manager  |  Stable pin: $STABLE_VERSION\"",
            "        if ($cur) {",
            "            $n = Parse-V $cur; $s = Parse-V $STABLE_VERSION",
            "            if     ($cur -eq $STABLE_VERSION) { Write-Host \"  OK  $cur (stable pin)\" -ForegroundColor Green }",
            "            elseif ($n -lt $s) { Write-Host \"  W   $cur < stable. Run: .\\openclaw-version-manager.ps1 switch\" -ForegroundColor Yellow }",
            "            else               { Write-Host \"  W   $cur > stable. Run Apply-fixes in installer GUI.\" -ForegroundColor Yellow }",
            "        } else { Write-Host \"  ERR Not installed. Run: .\\openclaw-version-manager.ps1 install\" -ForegroundColor Red }",
            "    }",
            "    \"install\" {",
            "        $a = @(\"-Version\", $Version); if ($Force) { $a += \"-ForceNewVersion\" }",
            "        & " + script_dir_install2 + " @a; exit $LASTEXITCODE",
            "    }",
            "    \"update\" {",
            "        $a = @(\"-TargetVersion\", $Version); if ($Force) { $a += \"-Force\" }",
            "        & " + script_dir_update + " @a; exit $LASTEXITCODE",
            "    }",
            "    \"switch\" {",
            "        Write-Host \"Uninstalling $($cur -replace '$null','none')...\"",
            "        npm uninstall -g openclaw 2>$null | Out-Null",
            "        $a = @(\"-Version\", $Version); if ($Force) { $a += \"-ForceNewVersion\" }",
            "        & " + script_dir_install2 + " @a; exit $LASTEXITCODE",
            "    }",
            "    \"list\" {",
            "        Write-Host \"Known versions:\"",
            "        Write-Host \"  OK  " + stable + "   STABLE PIN\" -ForegroundColor Green",
            "        Write-Host \"  W   2026.4.x   hardcoded 60s LLM timeout (undici preload mitigates)\" -ForegroundColor Yellow",
            "        Write-Host \"  W   2026.5.7   auto-update wipes openclaw.json (DECISION #44)\" -ForegroundColor Yellow",
            "        Write-Host \"  ERR 2026.5.18  exec schema strict; profile rejected (DECISION #43 RETIRED)\" -ForegroundColor Red",
            "        Write-Host \"  ?   latest     unknown -- run Apply-fixes after install\" -ForegroundColor DarkGray",
            "    }",
            "}",
        ]
        manager_ps1 = "\r\n".join(manager_lines) + "\r\n"
        ...
        scripts = {
            "install-openclaw.ps1":          install_ps1,
            "update-openclaw.ps1":           update_ps1,
            "openclaw-version-manager.ps1":  manager_ps1,
        }

        all_ok = True
        for fname, ps1_content in scripts.items():
            fpath = os.path.join(workspace_dir, fname)
            try:
                with open(fpath, "w", encoding="utf-8", newline="") as f:
                    f.write(ps1_content)
                self._log(f"  {fname} written to workspace \u2713", "SUCCESS")
            except Exception as e:
                self._log(f"  {fname}: {e}", "WARNING")
                all_ok = False
    ...
    def _build_soul_content(self, hw_profile: dict | None = None) -> str:
        ...
        Memory entry types written by LYRA at session end:
          [LEARNING] — triggered by error/failure events A–E (what went wrong)
          [CONTEXT]  — triggered by productive work events F–I (what was achieved
                       and should continue in a future session)
        Both types are written to memory/YYYY-MM-DD.md.
        [CONTEXT] entries are max 6 lines to minimise inference token overhead.
        """
        ...
        # Load configured primary and fallback model from openclaw.json.
        # These are injected into SOUL.md so Lyra always reflects the live config.
        ...
        try:
            ...
            primary_model  = _model_cfg.get("primary",   "ollama/glm-4.7-flash:latest")
            _fallbacks     = _model_cfg.get("fallbacks", [])
            fallback_model = _fallbacks[0] if _fallbacks else "ollama/qwen2.5:7b-instruct"
        except Exception:
            primary_model  = "ollama/glm-4.7-flash:latest"
            fallback_model = "ollama/qwen2.5:7b-instruct"
        primary_short  = primary_model.replace("ollama/", "")
        fallback_short = fallback_model.replace("ollama/", "")
...
f"  Model: {primary_short} (primary — konfiguriert via Installer → Lyra Config).\n"
...
"    exec: Invoke-RestMethod http://127.0.0.1:18789/health | ConvertTo-Json\n"
...
"\n"
f"Häufigste Ursache: {primary_short} hängt beim Laden wegen VRAM-Druck.\n"
"  Wenn VRAM beim Modell-Load belegt ist → Ollama meldet exit status 2 oder Timeout.\n"
"\n"
...
"\n"
"MODELL-FALLBACK (aus openclaw.json agents.defaults.model):\n"
f"  Primary:  {primary_short}  → aktuell konfiguriert\n"
f"  Fallback: {fallback_short} → agents.defaults.model.fallbacks[0]\n"
"  Wechsel:  Installer → Lyra Config → Primary LLM Dropdown\n"
"\n"
f"REGEL: Wenn {primary_short} 3x hintereinander Timeout → Fallback-Modell wählen.\n"
...
"  exit status 2 = VRAM-Crash. NICHT sofort Modell wechseln \u2014 erst Ollama neu starten.\n"
"\n"
"  Schritt 1 \u2014 Ollama neu starten via exec:\n"
"  ```powershell\n"
"  docker restart mein-ki-setup-ollama-1\n"
"  Start-Sleep -Seconds 15\n"
"  ```\n"
"\n"
"  Schritt 2 \u2014 Anfrage einmal wiederholen.\n"
"\n"
"  Schritt 3 \u2014 Schlaegt Retry erneut fehl \u2192 Fallback-Modell aus Config laden:\n"
"  ```powershell\n"
"  $cfg = Get-Content \"$env:USERPROFILE\\.openclaw\\openclaw.json\" -Raw | ConvertFrom-Json\n"
"  $fallback = $cfg.agents.defaults.model.fallbacks[0]\n"
"  $cfg.agents.defaults.model.primary = $fallback\n"
...
"\n"
"```powershell\n"
"Invoke-RestMethod http://127.0.0.1:18789/health | ConvertTo-Json\n"
"Invoke-RestMethod http://127.0.0.1:11434/api/tags | ConvertTo-Json\n"
"```\n"
"\n"
...
"\n"
"Kontext-Check beim Session-Start:\n"
"  Lese memory/YYYY-MM-DD.md der letzten 14 Tage auf [CONTEXT]-Einträge.\n"
"  Wenn der User ein Thema anspricht das in einem [CONTEXT]-Eintrag steht:\n"
...
"\n"
"Beispiel DNABERT-2: AutoTokenizer/AutoModel('zhihan1996/DNABERT-2-117M', trust_remote_code=True) → last_hidden_state.mean(dim=1)\n"
"\n"
...
"\n"
"PFLICHT am Session-Ende wenn eines dieser Ereignisse zutrifft:\n"
"  F. Ein komplexes Thema wurde untersucht das in einer Folge-Session fortgesetzt werden könnte\n"
"  G. Der User hat ein Projekt / Ziel erwähnt das noch nicht abgeschlossen ist\n"
"  H. Recherche oder Analyse wurde erstellt die als Grundlage für Folgearbeit dient\n"
"  I. Die Session wurde explizit offen gelassen ('machen wir nächstes Mal weiter')\n"
"\n"
"NICHT schreiben für:\n"
"  - Abgeschlossene Einzel-Fragen (Wetter, Berechnungen, einmalige Infos)\n"
"  - Sessions ohne nennenswerte inhaltliche Arbeit\n"
"\n"
"### [CONTEXT]-Format\n"
"\n"
"Datei: memory/YYYY-MM-DD.md  (selbe Datei wie [LEARNING])\n"
"\n"
"  [CONTEXT] YYYY-MM-DD: <Projektname / Thema in 5 Wörtern>\n"
"  Ziel:     <Was will der User langfristig erreichen? 1 Satz.>\n"
"  Stand:    <Was wurde erarbeitet / beschlossen? Max 2 Sätze.>\n"
"  Offen:    <Nächste Schritte oder offene Fragen. Max 2 Sätze.>\n"
"  Quellen:  <Relevante URLs oder Dateipfade — nur wenn wirklich hilfreich>\n"
"\n"
"Max. 6 Zeilen pro Eintrag — kein Fließtext, keine Ausschweifungen.\n"
"Wenn derselbe Kontext bereits in einer älteren Datei existiert:\n"
"  → Neuen [CONTEXT]-Eintrag schreiben (neuester Eintrag gilt)\n"
"  → NICHT den alten Eintrag löschen\n"
"\n"
"---\n"
"\n"
...
"\n"
"Wurde in dieser Session komplexe Arbeit geleistet die in Folge-Sessions fortgesetzt werden könnte?\n"
"\n"
"  F. Ein Thema untersucht das nicht vollständig abgeschlossen ist\n"
"  G. User hat ein Projekt / Ziel erwähnt das weiterverfolgt werden soll\n"
"  H. Recherche / Analyse durchgeführt die als Grundlage weiterer Arbeit dient\n"
"  I. Session explizit offen gelassen ('nächstes Mal weiter')\n"
"\n"
"→ JA zu einem: [CONTEXT]-Eintrag in memory/YYYY-MM-DD.md schreiben.\n"
"→ NEIN zu allen: Nichts schreiben.\n"
"\n"
"WICHTIG: [CONTEXT]-Eintrag ist KURZ (max. 6 Zeilen).\n"
"         Kein Transkript. Nur Essenzielles für Folge-Sessions.\n"
"\n"
"### Regeln\n"
"\n"
"NICHT schreiben für Routine-Sessions ohne Ereignisse A–E (LEARNING) oder F–I (CONTEXT).\n"
...
"  exec: powershell -Command \"Invoke-RestMethod http://127.0.0.1:18789/health | ConvertTo-Json\"\n"
...
"   Tritt auf wenn direkt nach Gateway-Start ein Modell geladen wird,\n"
"   waehrend VRAM noch von anderem Prozess belegt ist.\n"
"   Ollama meldet: 'gpu VRAM usage didn't recover within timeout'\n"
"   Diagnose: nvidia-smi (VRAM-Belegung pruefen)\n"
"   Fix:\n"
"     1. docker restart mein-ki-setup-ollama-1  (VRAM leeren)\n"
"     2. Start-Sleep -Seconds 15\n"
"     3. Anfrage wiederholen\n"
"     Erst wenn Restart+Retry fehlschlaegt: Fallback-Modell waehlen (siehe unten).\n"
"\n"
...
"   Fix:\n"
f"     ollama rm {primary_short}\n"
f"     ollama pull {primary_short}\n"
"\n"
...
"\n"
"Fallback (nur wenn Restart+Retry fehlschlaegt):\n"
f"   Konfiguriertes Fallback-Modell: {fallback_short}\n"
"   Wechsel: Installer \u2192 Lyra Config \u2192 Primary LLM Dropdown.\n"
f"   Sobald VRAM wieder frei: nvidia-smi pruefen, dann zu {primary_short} zurueckkehren.\n"
"\n"
"---\n"
"\n"
...
<Task version="1.2" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">
  ...
  <Actions>
    ...
    def fix_openclaw_installation(self):
        ...
        # DECISION #45: stable pin first, GitHub .tgz second, then latest fallbacks.
        stable  = self.OPENCLAW_STABLE_VERSION
        tgz_url = self.OPENCLAW_GITHUB_FALLBACK
        sources = [
            (f"npm openclaw@{stable} (stable pin, DECISION #45)",
             f"npm install -g openclaw@{stable}"),
            (f"GitHub .tgz openclaw@{stable} (DECISION #45 fallback)",
             f"npm install -g {tgz_url}"),
